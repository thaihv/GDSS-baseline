...
function getFeature(WFSServer, layerName, fieldName, value) {
    ...
    getFeatureUrl += "&srsName=CCI:8880";   // 지도상에 표출한 좌표계 
...
function moveFeature(WFSServer, layerName, fieldName, value) {
    ...
    params += "&srsName=CCI:8880";   // 지도상에 표출한 좌표계 

   OpenLayers.loadURL("./proxy.web", "?url=" + escape(WFSServer + params), this, gmlMove, goUrl);
...
function gmlMove(response) {
 ...
 //  alert("gmlMove" + response.responseText);
	...
	if (features != null && features.length > 0) {
       ...
       tnSelectionLayer.addFeatures(features);
       // alert(bounds);
       map.zoomToExtent(bounds);
       tnmap.zoomToExtent(bounds);
        
        
      var ik = parseInt(map.getScale()); //현재 스케일 보기 
      var tnik = parseInt(tnmap.getScale()); //현재 스케일 보기 
        ...
        var tniScale= parseInt(tnik+iSum);
        
        map.zoomToScale(iScale, true);
        tnmap.zoomToScale(tniScale, true);
