...
Ext.define('GeoExt.tree.LayerNode', {
    ...
    onCheckChange: function() {
        ...
        // Chau Bui add for check "Ảnh upload" layer
		if(node.data.text === "Ảnh upload") {
			if (checked == true) {
				var listNodes1 = GLB_TreeLayer.view.node.childNodes[0].childNodes;
				var listNodes2 = GLB_TreeLayer.view.node.childNodes[1].childNodes;
				var listNodes = listNodes1.concat(listNodes2);
				for(var i=0; i<listNodes.length; i++) {
					if (!(listNodes[i].data.text === "Ảnh upload")) {
						listNodes[i].checked = false;
						var layer = listNodes[i].get('layer');
		...
		if(node.data.text === "Editor") {
			if (checked == true) {
				var listNodes1 = GLB_TreeLayer.view.node.childNodes[0].childNodes;
				var listNodes2 = GLB_TreeLayer.view.node.childNodes[1].childNodes;
				var listNodes = listNodes1.concat(listNodes2);
				for(var i=0; i<listNodes.length; i++) {
					if (!(listNodes[i].data.text === "Editor") && !(listNodes[i].data.text === "Snapping")) {
						listNodes[i].checked = false;
						var layer = listNodes[i].get('layer');
						layer.setVisibility(false)
					}
			...
			if (checked == true && node.data.text != "Snapping") {
				var listNodes1 = GLB_TreeLayer.view.node.childNodes[0].childNodes;
				var listNodes2 = GLB_TreeLayer.view.node.childNodes[1].childNodes;
				var listNodes = listNodes1.concat(listNodes2);
				for(var i=0; i<listNodes.length; i++) {
					if (listNodes[i].data.text === "Editor") {
						listNodes[i].checked = false;
						var layer = listNodes[i].get('layer');
						layer.setVisibility(false);
						break;
