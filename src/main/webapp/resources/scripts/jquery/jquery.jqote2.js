...
(function($) {
    ...
    $.extend({
        ...
        jqotec: function(template, t, alias) {
            ...
            /// in metaworks, only accepts original templates
...
//		in metaworks, the data scope is in the data[j] so that in the template, 'this.' expression is not required.
            str = 'try{with(data[j]){' +
                ('var out="";'+str+';return out;')
                    .split("out+='';").join('')
                        .split('var out="";out+=').join('var out=') +
                '}}catch(e){throw e;e.type="'+JQOTE2_TMPL_EXEC_ERROR+'";e.args=arguments;e.template=arguments.callee.toString();throw e;}';

            try {
                var fn = new Function('i, j, data, fn', str);
            } catch ( e ) { alert(str); throw e; raise(e, {type: JQOTE2_TMPL_COMP_ERROR}); }
