...
Ext.define('Ext.ux.upload.ItemGridPanel', {
    ...
    requires : [
        'Ext.selection.CheckboxModel', 'Ext.ux.upload.Store', 'Ext.ux.upload.FileDownloader'
    ],
    ...
    initComponent : function() {
        ...
        Ext.apply(this, {
            ...
            columns : [
                {
                	...
                	header: 'category',
                	width: 150,
                	renderer: this.getDisplayOnArFileCategoryStore
                }, {
                    id : 'filename',
                    dataIndex : 'filename',
                	...
                	dataIndex: 'checkvalidation',
                	header: 'validation',
                	width: 70,
                	renderer: this.getDisplayDataByCheckBox
                	/*
                    dataIndex : 'type',
            ...
            listeners: {
				cellclick: this.onCellClickFileView,
				scope: this
            }
        });
    ...
    onCellClickFileView : function(iView, iCellEl, iColIdx, iRecord, iRowEl, iRowIdx, iEvent){
    	console.log( 'itemGrid CellClick start---------------------------------');
		console.log( iRecord);
		var fieldName = iView.getGridColumns()[iColIdx].dataIndex;
		if( fieldName == 'filename'){
			//this.onLoadFile( iRecord);
			//console.log(Ext.ComponentQuery.query('filedownloader[itemid=filedownloader]'));
			var fileView = Ext.ComponentQuery.query('filedownloader[itemid=filedownloader]')[0];
			fileView.load({
				url: '/land-office/getJsonFileData.do',
	    		params: {
	    			type: 'viewfile',
	    			fileid: iRecord.get( 'fileid')
	    		} 
			});
		}
		console.log( 'itemGrid CellClick end---------------------------------');
    },
    ...
    loadQueueItems : function(items) {
        var data = [];
        var i;
        console.log( ' loadQueueItems start ------------------------------ ');
        console.log( 'items length : ' + items.length);
        for (i = 0; i < items.length; i++) {
        	//console.log( items[i].getFilename() + ' : ' + items[i].getStatus() + ' : ' + items[i].getProgressPercent());
            data.push([
				items[i].getFileCategory(),
                items[i].getFilename(),
                items[i].getSize(),
                items[i].getCheckValidation(),
                //items[i].getType(),
                items[i].getStatus(),
        ...
        console.log( ' loadQueueItems end ------------------------------ ');
    ...
    statusRenderer : function(value, metaData, record, rowIndex, colIndex, store) {
        ...
        console.log( 'status renderer start -------------------');
        console.log( 'value : ' + value);
        console.log( 'status renderer end -------------------');
        return value;
    },
    ...
    //moon9 function
    ,
    getDisplayOnArFileCategoryStore : function( inputValue){
    	var arFileCategoryStore = Ext.ComponentQuery.query('combobox[itemid=fileCategoryField]')[0].getStore();
    	var num = arFileCategoryStore.getRange().length;
    	var i;
    	for( i=0; i<num; i++){    		
    		if(arFileCategoryStore.getAt(i).getData().categoryValue == inputValue){
    			return arFileCategoryStore.getAt(i).getData().categoryDisplay;
    		}
    	}
    },
    getDisplayDataByCheckBox: function( input){
    	if( input == 'true'){
    		return 'yes';
    	}else{
    		return 'no';
